...
#[test]
fn test_create_product_and_add_step_with_roles() {
    let e = Env::default();
    e.mock_all_auths();

    let admin = Address::generate(&e);
    let manufacturer = Address::generate(&e);
    let shipper = Address::generate(&e);

    let token = create_token(&e, &admin);

    // Grant roles
    token.grant_role(&manufacturer, &(1_u32)); // Manufacturer
    token.grant_role(&shipper, &(2_u32)); // Shipper

    // Manufacturer creates product
    token.create_product(&"PROD1".into_val(&e), &"SN1".into_val(&e), &manufacturer, &"Factory".into_val(&e));

    // Verify product exists and initial status is Production (0)
    let status = token.get_current_status(&"PROD1".into_val(&e));
    assert_eq!(status, 0);

    // Shipper adds a shipping step
    let meta: Map<_, _> = Map::new(&e);
    token.add_step(&"PROD1".into_val(&e), &1_u32, &"OnRoute".into_val(&e), &shipper, &Option::<String>::None, &meta);

    // History length should be >= 2 now
    let history = token.get_product_history(&"PROD1".into_val(&e));
